...
const ModalAdsTable = ({ onClose }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post('http://192.34.63.158:4000/adds/getall');
  ...
  const handleSubmitForm = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.34.63.158:4000/adds/add', formDataToSend);
  ...
  const handleSubmitFormEditTeam = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.34.63.158:4000/adds/update', formDataToSend);
      if (response.status >= 200 && response.status < 300) {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.post('http://192.34.63.158:4000/adds/delete', { id });
      console.log("Status:", response.status);
            ...
            {data.map((item) => (
              <tr key={item._id}>
                ...
                <td>
                  ...
                  <img src={`http://192.34.63.158:4000/add/${item.image_url}`} style={{ width: '100px' }} />

...
const AboutAdmin = () => {
  ...
  const loadTeam = async () => {
    ...
    const { data } = await axios.post(`http://192.34.63.158:4000/about/offer/getAll`,
    )
  ...
  const handleDelete = async (id) => {
    ...
    const response = await axios.delete(`http://192.34.63.158:4000/about/offer/delete/${id}`);
  ...
  // const loadTeam = async () => {
  //   try {
  //     const response = await axios.get('http://192.34.63.158:4000/about/team/getAll');
  //     const { data } = response;
  //     if (data.status === true) {
  //       setData(data.data);
  //     } else {
  ...
  const handleSubmitFormEditTeam = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.34.63.158:4000/about/offer/update', formDataToSend);
      if (response.status >= 200 && response.status < 300) {
        setEditingTeamMember(false)
        window.alert(response.data.message);
        // Reload the team data after successful update
  ...
  const handleSubmitForm = async (e) => {
    ...
    try {
      ...
      console.log("formDataToSend:", formDataToSend);
      const response = await axios.post('http://192.34.63.158:4000/about/offer/add', formDataToSend);
            ...
            <tbody>
              {data && data.map((item) => (
                <tr className='tr' key={item.id}>
                  ...
                  <td>
                    <img
                      src={`http://192.34.63.158:4000/offer/${item.logo}`}
                      alt={item.title}

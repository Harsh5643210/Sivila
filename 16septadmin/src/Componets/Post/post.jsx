...
const Post = () => {
  ...
  const handleDeleteRow = async (id) => {
    try {
      ...
      const response = await axios.post('http://192.34.63.158:4000/user/post/delete', { postId: id });
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://192.34.63.158:4000/user/post/all');
          ...
          <tbody>
            {data && data.map((item) => (
              <tr key={item._id}>
                ...
                <td>
                  {<img src={`http://192.34.63.158:4000/posts/${item.postUrl}`} alt={item.location_name} style={{ width: '100px' }} />}
                </td>

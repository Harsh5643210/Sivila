...
const Adventure = () => {
  ...
  const handleDeleteRow = async (id) => {
    try {
      ...
      const response = await axios.post('http://192.34.63.158:4000/adventure/delete', { id: id });
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post('http://192.34.63.158:4000/adventure/getall');
          ...
          <tbody>
            {data && data.map((item) => (
              <tr key={item._id}>
                ...
                <td>
                  {<img src={`http://192.34.63.158:4000/adventure/${item.image_url}`} alt={item.location_name} style={{ width: '100px' }} />}
                </td>
